...
| 노선 종류 | UI | 저장 값 |
|---|---|---|
| 일반선 | [상선][하선][상하선][역구내] 버튼 | `['상선']` / `['하선']` / `['상선','하선']` |
| 역구내 | [역구내] 버튼 (보라색) | `['상선','하선']` (isYardWork 로컬 상태만 별도 관리) |
| 고속선 | T1~T8 체크박스 | `['T1','T2',...]` |

### 전차선 단전 표시 규칙

차단종류와 무관하게 본선 노선 선택 시 항상 표시. 노선 선택 즉시 변전소 목록 자동 로드.  
`section_note`는 `변전소시작~변전소종료 (단전구간)` 형식으로 자동 구성.
