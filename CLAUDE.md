...
1. **`index.html` viewport에서 `maximum-scale=1.0, user-scalable=no` 제거 또는 단순화 금지**
   ```html
   <!-- 이 상태를 유지할 것 — 절대 변경 금지 -->
   <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" />
   ```
   이유: Mac 트랙패드 핀치 확대 시 브라우저 전체 줌 → 사이드바가 확대된 채 고정되고
   D3 맵 축소로는 복구 불가. `user-scalable=no`가 없으면 반드시 재발한다.

2. **새 역할(role) 추가 시 `BlockMapPage.tsx` 역할 분기 미갱신 금지**
   ```typescript
   // 현재 상태 — 신규 role 추가 시 반드시 전체 검토
   const isSuperuser    = user?.role === 'system_superuser';
   const isBlockManager = user?.role === 'block_manager';
   const isOrgSelector  = isSuperuser || isBlockManager;
   const isOrgUser      = !isOrgSelector && user?.organization_id != null;
   ```
   이유: `block_manager`(organization_id=null) 추가 시 조건 미갱신으로
   "전국 조망/지역본부 선택" 드롭다운이 사라지는 버그 발생 이력 있음.
