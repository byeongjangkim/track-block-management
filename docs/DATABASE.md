...
| `facility_point` | `rail_facilities.kp_start` (점 시설물) | 0 (예정) |
| `facility_start/end` | `rail_facilities.kp_start/end` (구간 시설물) | 0 (예정) |
...
CREATE TABLE block_orders (
    ...
    -- 노선·위치
    direction        TEXT(4)  NOT NULL,          -- 'UP' | 'DOWN' | 'BOTH' (기지 전체 작업)
...
| `direction = 'BOTH'` | 기지 전체 작업 시 사용. 상·하선 방향 무관 |
...
- `direction` (block_orders): `'UP'` / `'DOWN'` / `'BOTH'`. 기지 전체 작업 시 `'BOTH'` 허용
